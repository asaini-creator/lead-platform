...
- **Database:** lightweight data store in `db/` [ADD: confirm — e.g. JSON file-based, in-memory, or lowdb — no DB driver is in package.json, so specify what `db/` actually does]
...
```
lead-platform/
├── db/            # Data store (leads, users, activity)
...
- **Live URL:** [ADD LINK]
- **Admin login:** `[ADD email]` / `[ADD password]`
- **Member login:** `[ADD email]` / `[ADD password]`
...
- [ADD any assumptions you made, e.g. specific lead statuses chosen, what counts as an "activity trail" entry, etc.]
